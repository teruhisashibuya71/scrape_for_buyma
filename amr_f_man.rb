...
module AmrFarfetchMan
    ...
    def amr_farfetch_return_category_page_url(attack_site_url, search_category)
        ...
        if search_category == "服" then
            amr_farfetch_clothing_url = attack_site_url.sub(/scale=282/, 'category=135967')
            return amr_farfetch_clothing_url
        elsif search_category == "靴" then
            amr_farfetch_shoes_url = attack_site_url.sub(/scale=282/, 'category=136301')
            return amr_farfetch_shoes_url
        elsif search_category == "バッグ" then
            amr_farfetch_bag_url = attack_site_url.sub(/scale=282/, 'category=135971')
            return amr_farfetch_bag_url
        else
            amr_farfetch_accessori_url = attack_site_url.sub(/scale=282/, 'category=135973')

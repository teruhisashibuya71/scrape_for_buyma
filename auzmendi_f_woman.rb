...
module AuzmendiFarfetchWoman
    ...
    def auzmendi_farfetch_onetime_crowl(doc, auzmendi_farfetch_target_price)
    ...
    def auzmendi_farfetch_crowl(attack_site_url, target_price, category)
        #価格が4桁の場合は調整
        ...
        if (products.size == 0)
            ...
            #初回クロール
            auzmendi_farfetch_onetime_crowl(doc, target_price)
            ...
            while doc.css('li[data-testid="productCard"]').size == 90 do
                ...
                doc = auzmendi_farfetch_make_doc(auzmendi_farfetch_categorized_url)
                #2ページ目をクロール
                auzmendi_farfetch_onetime_crowl(doc, auzmendi_farfetch_target_price)
            end
        end
    end
end

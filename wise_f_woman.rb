...
module WiseFarfetchWoman
    ...
    def wise_farfetch_return_category_page_url(attack_site_url, search_category)
        if search_category == "服" then
            wise_farfetch_clothing_url = attack_site_url.sub(/scale=274/, 'category=135967')
            return wise_farfetch_clothing_url
        elsif search_category == "靴" then
            wise_farfetch_shoes_url = attack_site_url.sub(/scale=274/, 'category=136301')
            return wise_farfetch_shoes_url
        elsif search_category == "バッグ" then
            wise_farfetch_bag_url = attack_site_url.sub(/scale=274/, 'category=135971')
            return wise_farfetch_bag_url
        else
            wise_farfetch_accessori_url = attack_site_url.sub(/scale=274/, 'category=135973')
    ...
    def wise_farfetch_crawl(attack_site_url, target_price, category)
        ...
        if (products.size == 0)
            ...
            #レディースと比較して2文字少ないので
            wise_farfetch_categorized_url = wise_farfetch_categorized_url.insert(68, "page=#{page_number}&")
            while doc.css('li[data-testid="productCard"]').size == 90 do
                ...
                wise_farfetch_categorized_url[73] = "#{page_number}"

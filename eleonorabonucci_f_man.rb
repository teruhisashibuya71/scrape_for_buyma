...
#処理は大丈夫なはず
module EleonorabonucciFarfetchMan
    ...
    def elenora_farfetch_return_category_page_url(attack_site_url, search_category)
        ...
        if search_category == "服" then
            elenora_farfetch_clothing_url = attack_site_url.sub(/scale=282/, 'category=136330')
            return elenora_farfetch_clothing_url
        elsif search_category == "靴" then
            elenora_farfetch_shoes_url = attack_site_url.sub(/scale=282/, 'category=135968')
            return elenora_farfetch_shoes_url
        elsif search_category == "バッグ" then
            elenora_farfetch_bag_url = attack_site_url.sub(/scale=282/, 'category=135970')
            return elenora_farfetch_bag_url
        else
            elenora_farfetch_accessori_url = attack_site_url.sub(/scale=282/, 'category=135972')
    ...
    def elenora_farfetch_crawl(attack_site_url, target_price, category)
        #価格が4桁の場合は調整
        ...
        #初回クロール
        elenora_farfetch_onetime_crawl(doc, target_price)
        ...
        elenora_farfetch_categorized_url = elenora_farfetch_categorized_url.insert(60, "page=#{page_number}&")
        while doc.css('[data-component="ProductCard"]').size == 90 do
            ...
            doc = elenora_farfetch_make_doc(elenora_farfetch_categorized_url)
            #2ページ目をクロール
            elenora_farfetch_onetime_crawl(doc, elenora_farfetch_target_price)
        end
    end
end

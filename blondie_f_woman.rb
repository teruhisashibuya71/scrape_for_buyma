...
3.65行目のURL置換の桁数を変更 55+ショップ文字数
4.68行目のページ数の数値を変更する 65の数字+5=
=end
...
module BlondieFarfetchWoman
    ...
    def blondie_farfetch_make_doc(blondie_farfetch_categorized_url)
        charset = nil
        html = open(blondie_farfetch_categorized_url) do |f|
            charset = f.charset
            f.read
        end
    ...
    def blondie_farfetch_onetime_crawl(attack_site_url, blondie_farfetch_target_price, category, doc)
    ...
    def blondie_farfetch_crawl(attack_site_url, target_price, category)
        ...
        blondie_farfetch_onetime_crawl(blondie_farfetch_categorized_url, target_price, category, doc)
        ...
        while doc.css('li[data-testid="productCard"]').size == 90 do
            page_number += 1
            blondie_farfetch_categorized_url[67] = "#{page_number}" #
            doc = blondie_farfetch_make_doc(blondie_farfetch_categorized_url)
            doc.css('li[data-testid="productCard"]').each do |node|
                item_price = node.css('span[data-testid="price"]').inner_text

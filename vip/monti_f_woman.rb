...
module MontiFarfetchWoman
    ...
    def monti_farfetch_return_category_page_url(attack_site_url, search_category)
        ...
        if search_category == "服" then
            monti_farfetch_clothing_url = attack_site_url.sub(/scale=274/, 'category=135967')
            return monti_farfetch_clothing_url
        elsif search_category == "靴" then
            monti_farfetch_shoes_url = attack_site_url.sub(/scale=274/, 'category=136301')
            return monti_farfetch_shoes_url
        elsif search_category == "バッグ" then
            monti_farfetch_bag_url = attack_site_url.sub(/scale=274/, 'category=135971')
            return monti_farfetch_bag_url
        else
            monti_farfetch_accessori_url = attack_site_url.sub(/scale=274/, 'category=135973')
    ...
    def monti_farfetch_crawl(attack_site_url, target_price, category)
        ...
        while doc.css('[data-component="ProductCard"]').size == 90 do
            ...
            monti_farfetch_categorized_url[65] = "#{page_number}"

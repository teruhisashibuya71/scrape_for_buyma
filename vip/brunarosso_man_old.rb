...
module BrunarossoManOld
    ...
    def brunarosso_crawl_selenium(brand_home_url, search_price, search_category, target_brand)
        ...
        #商品ページにはクリックでしか移動できないので、引数のtarget_brandの値に応じでクリックするブランドを変更する
        
        case target_brand
        when "ALEXANDER_MCQUEEN"
            driver.find_element(:xpath, '//*[@id="__nuxt"]/div/div[3]/div[2]/div/div/div[2]/div[3]/ul/li[6]/a').click
        when "BALENCIAGA"
            driver.find_element(:xpath, '//*[@id="__nuxt"]/div/div[3]/div[2]/div/div/div[2]/div[3]/ul/li[14]/a').click
        end

        #商品リストが表示されるまで待つ
        wait.until { driver.find_elements(:class, 'CategoryList').displayed? }
        #ドロップダウンメニューをクリック
